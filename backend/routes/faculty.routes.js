...
const { storeReferenceDocument, purgeAssignmentVectors } = require('../utils/ragEvaluator');
...
router.post('/classrooms/:classroomId/assignments', authMiddleware('Faculty'), async (req, res) => {
    try {
        const { classroomId } = req.params;
        const { title, description, dueDate, maxMarks, assignmentFile, filePath, referenceFileUrl, referenceFilePath, handwrittenRequired } = req.body;
        const facultyId = req.user._id;
        ...
        const assignment = new Assignment({
            ...
            createdBy: facultyId,
            handwrittenRequired: parseBoolean(handwrittenRequired),
            referenceMaterial: referenceFileUrl ? {
        ...
        if (referenceFileUrl) {
            try {
                const { chunkCount } = await storeReferenceDocument(assignment._id, referenceFileUrl);
                ...
                console.error('Reference document ingestion error:', refError.message);
            }
        }
...
router.delete('/assignments/:assignmentId', authMiddleware('Faculty'), async (req, res) => {
    try {
        ...
        try {
            await purgeAssignmentVectors(assignment._id);
        } catch (vectorError) {
            console.error('Error purging assignment vectors:', vectorError.message);
        }
...
router.put('/assignments/:assignmentId', authMiddleware('Faculty'), async (req, res) => {
    try {
        ...
        const { title, description, dueDate, maxMarks, assignmentFile, referenceFileUrl, referenceFilePath, handwrittenRequired } = req.body;
        const facultyId = req.user._id;
        ...
        if (handwrittenRequired !== undefined) {
            assignment.handwrittenRequired = parseBoolean(handwrittenRequired, assignment.handwrittenRequired);
        }
        if (assignmentFile) {
            assignment.assignmentFile = assignmentFile;
        }

        let referenceReindexed = false;
        if (referenceFileUrl) {
            assignment.referenceMaterial = {
                url: referenceFileUrl,
                filePath: referenceFilePath || '',
            ...
            try {
                await purgeAssignmentVectors(assignment._id);
                const { chunkCount } = await storeReferenceDocument(assignment._id, referenceFileUrl);
                assignment.referenceMaterial.chunkCount = chunkCount;
                assignment.referenceMaterial.indexedAt = new Date();
                referenceReindexed = true;
            } catch (vectorError) {
                console.error('Reference reindex error:', vectorError.message);
            }
        }
