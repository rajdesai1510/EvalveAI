...
const GEMINI_DETECTION_MODEL = process.env.GEMINI_DETECTION_MODEL || 'gemini-1.5-flash-latest';
const GEMINI_EMBED_MODEL = process.env.GEMINI_EMBED_MODEL || 'text-embedding-004';
const DEEPSEEK_API_KEY = process.env.DEEPSEEK_API_KEY;
const DEEPSEEK_OCR_ENDPOINT = process.env.DEEPSEEK_OCR_ENDPOINT || 'https://api.deepseek.com/v1/images/ocr';
 ...
 */
const extractTextFromPdf = async (pdfUrl, options = {}) => {
    ...
    try {
        ...
        let ocrUsed = false;
        let pdfText = await extractPDFTextWithParser(tempFilePath);
        console.log('Text extracted successfully, length:', pdfText.length, 'characters');

        if (pdfText.length < 50 && DEEPSEEK_API_KEY) {
            console.log('Falling back to DeepSeek OCR for handwritten/low-text PDF');
            const ocrText = await runDeepSeekOCR(response.data);
            if (ocrText?.trim()) {
                pdfText = ocrText.trim();
                ocrUsed = true;
                console.log('DeepSeek OCR extraction succeeded, length:', pdfText.length);
            }
        }
        ...
        return withMetadata ? { text: pdfText, ocrUsed } : pdfText;
    } catch (error) {
        console.error('Error extracting text from PDF:', error);
        return withMetadata ? { text: '', ocrUsed: false } : "";
    }
};
...
/**
 * Gets embedding vector for the given text using Gemini Embeddings API
 * @param {String} text
...
const getTextEmbedding = async (text) => {
    ...
    try {
        const response = await axios.post(
            `https://generativelanguage.googleapis.com/v1/models/${GEMINI_EMBED_MODEL}:embedContent?key=${GEMINI_API_KEY}`,
            {
        ...
        // Return default embedding size for text-embedding-004 (768 dimensions)
        return Array(768).fill(0);
    }
};

const runDeepSeekOCR = async (pdfBuffer) => {
    if (!DEEPSEEK_API_KEY || !pdfBuffer) return '';
    try {
        ...
        return response.data?.data?.text || response.data?.result || '';
    } catch (error) {
